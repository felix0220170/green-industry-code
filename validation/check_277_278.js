...
console.log('开始检查277和278相关数据：');

// 查找所有与277或278相关的行
const rows277_278 = data.filter(row => {
  const subcategory = row['中类'] || '';
  const item = row['小类'] || '';
  
  return subcategory.includes('277') || subcategory.includes('278') || 
         item.includes('277') || item.includes('278');
});

console.log('找到与277/278相关的行：', rows277_278.length);

rows277_278.forEach((row, index) => {
  console.log(`\n第${index + 1}行：`);
  console.log('  门类:', row['门类']);
  console.log('  大类:', row['大类']);
  console.log('  中类:', JSON.stringify(row['中类'])); // 使用JSON.stringify查看换行符
  console.log('  小类:', JSON.stringify(row['小类'])); // 使用JSON.stringify查看换行符
...
if (documentXml) {
  ...
  for (const row of rows) {
    if (row.includes('277') && row.includes('278')) {
      console.log('\n找到包含277和278的Word原始行：');
      // 提取单元格内容
  ...
  if (!found) {
    console.log('未找到同时包含277和278的Word原始行');
  }
}
